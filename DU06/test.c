...
int isThere(char * string){
  char * helpstr = string;
  while(helpstr[0] != '\0'){
    if (helpstr[0] != ' ' && helpstr[0] != '\n' && helpstr[0] != '\t') return 1;
    helpstr++;
  }
  return 0;
}
...
char * tmpch = (char *)malloc( findLengh((char *)src) * sizeof(char));
char *poich = (char*)src;

int count = 0, flag = 0, newLine = 1, firstWord = 1, when = 0, strl = 0;
int widthLine = width;
tmpch[0] = '\0';
outch[0] = '\0';
while (isThere(poich)){
  ...
  strl = (int)strlen(tmpch);
  (strl>widthLine)?(newLine=1):(newLine=0);
    if (count>(strl+1)){

      for (int i = 0; i < (count-strl); i++ ){
        if (src[when+i]=='\n')
          flag++;
        if (flag == 2){
